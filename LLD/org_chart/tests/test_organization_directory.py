...
def directory() -> OrganizationDirectory:
    """Build the Company/Engineering/Search/Data/SRE fixture with three employees.

    Called by: pytest fixture injection into tests requesting directory.
    Returns: OrganizationDirectory populated with the test hierarchy.
    Example (with test-local values): OrganizationDirectory(InMemoryDirectoryRepository())
    """
...
def test_employees_in_same_group(directory: OrganizationDirectory) -> None:
    """Verify the scenario: employees in same group.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): assert directory.closest_common_group([101, 103]).group_id == 3
    """
...
def test_employees_in_sibling_groups(directory: OrganizationDirectory) -> None:
    """Verify the scenario: employees in sibling groups.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): assert directory.closest_common_group([101, 102]).group_id == 2
    """
...
def test_single_employee_returns_own_group(directory: OrganizationDirectory) -> None:
    """Verify the scenario: single employee returns own group.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): assert directory.closest_common_group([101]).group_id == 3
    """
...
def test_employee_move_changes_result(directory: OrganizationDirectory) -> None:
    """Verify the scenario: employee move changes result.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): assert directory.closest_common_group([101, 102]).group_id == 1
    """
...
def test_group_move_changes_result(directory: OrganizationDirectory) -> None:
    """Verify the scenario: group move changes result.
...
def test_group_move_rejects_cycle(directory: OrganizationDirectory) -> None:
    """Verify the scenario: group move rejects cycle.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): the exception expectation in the test body
    """
...
def test_empty_employee_list_is_rejected(directory: OrganizationDirectory) -> None:
    """Verify the scenario: empty employee list is rejected.
...
def test_unknown_employee_is_rejected(directory: OrganizationDirectory) -> None:
    """Verify the scenario: unknown employee is rejected.
...
def test_unknown_parent_is_rejected(directory: OrganizationDirectory) -> None:
    """Verify the scenario: unknown parent is rejected.
...
def test_duplicate_employee_is_rejected(directory: OrganizationDirectory) -> None:
    """Verify the scenario: duplicate employee is rejected.
...
def test_separate_trees_have_no_common_group(
    ...
    """Verify the scenario: separate trees have no common group.
...
def test_parallel_employee_adds_are_safe(
    ...
    """Verify the scenario: parallel employee adds are safe.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): assert all((directory.get_employee(employee_id).employee_id == employee_id for employee_id in employee_ids))
    """
    from concurrent.futures import ThreadPoolExecutor

    def add(employee_id: int) -> None:
        """Insert one uniquely identified employee from a concurrent worker.

        Called by: the enclosing test or its thread-pool callback.
        Returns: None; assertions raise on failure.
        Example (with test-local values): directory.add_employee(Employee(employee_id, f'E{employee_id}', 3))
        """
...
def test_repository_contract_rejects_incomplete_implementation() -> None:
    """Verify the scenario: repository contract rejects incomplete implementation.
...
def test_domain_error_has_stable_code(directory) -> None:
    """Verify the scenario: domain error has stable code.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): assert caught.value.code == 'not_found'
    """
...
def test_successful_move_logs_ids_without_personal_details(directory, caplog) -> None:
    """Verify the scenario: successful move logs ids without personal details.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): assert 'Moved employee id=101 from=3 to=4' in caplog.text
    """
    import logging
...
def test_failed_move_preserves_state_and_has_no_success_log(directory, caplog) -> None:
    """Verify the scenario: failed move preserves state and has no success log.

    Called by: pytest discovery with the fixtures arranged below.
    Returns: None; failed expectations raise assertion errors.
    Example expectation (using the fixture below): assert directory.get_employee(101).group_id == 3
    """
    import logging
