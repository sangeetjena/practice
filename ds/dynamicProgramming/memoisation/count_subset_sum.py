...
"""
def count_sub_set_sum(arr,sum, i, memo):
    # base case
    if sum == 0:
        return 1
    if i<0:
        return 0
    if ((sum,i) in memo.keys()):
        return memo[(sum,i)]
    ret = count_sub_set_sum(arr,sum, i-1, memo) + count_sub_set_sum(arr,sum-arr[i], i-1, memo)
    # memoise result of subset sum for present sum value and i value sothat if another time recursion call happen with
    # same value, it will return the value without doing calculation again.
    memo[(sum,i)] = ret
    return ret
